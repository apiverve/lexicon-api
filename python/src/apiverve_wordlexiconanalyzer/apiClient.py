...
class LexiconAPIClientError(Exception):
...
class ValidationError(LexiconAPIClientError):
...
class LexiconAPIClient:
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the LexiconAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises LexiconAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[LexiconAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns WordLexiconAnalyzerData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises LexiconAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise LexiconAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise LexiconAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise LexiconAPIClientError(f"Request failed: {str(e)}")
